 ...
 * Created at: 11/09/2006 20:53:20
 *
 * $Revision: 1.10 $
...
public class ClusterNode {
	...
	public void start(String[] args) {
		args.getClass(); // Unused parameter, just kept for public API
			...
			try {
				for (Enumeration e = NetworkInterface.getNetworkInterfaces(); e.hasMoreElements();) {
					...
					for (Enumeration ips = itf.getInetAddresses(); ips.hasMoreElements();) {
						...
						if (ip.getHostAddress().indexOf(':') == -1) { // Filter IPv6
	...
	private void commandUpdate() {
		...
		try {
			...
			if (message != null && message.length() > 0) {
				System.out.println(message); // Message from update server
				if (version == null || version.length() == 0) {
					return; // Aborts operation
				}
			...
			final String message = ""
				+ "It was not possible to check for updates on the web.\n"
				+ "Error: " + e.getMessage() + "\n"
				+ "\n"
	...
	private void updateVersion(String version, String jarUrl) {
		...
		if (confirm("Do you want to update this node? (Will restart the application)")) {
			...
			try {
				...
				final String message = ""
					+ "An error happened while performing the update process.\n"
	...
	private void commandExit() {
		new Thread("commandExit timeout thread") {
			public void run() {
				...
				System.exit(EXIT_NORMAL_SHUTDOWN); // Forces exit after 5 seconds
			}
		} .start();
		try {
	...
	private void commandRestart() {
		new Thread("commandRestart timeout thread") {
			public void run() {
				...
				System.exit(EXIT_NORMAL_RESTART); // Forces exit after 5 seconds
			}
		} .start();
		try {
			System.out.println("Stopping process node...");
	...
	private void showHelp() {
		final String s = ""
			+ "# OpenRDS " + Version.getVersion() + " (Built on " + Version.getBuildDate() + ")\n"
	...
	private String readLine() {
		try {
			...
			if (s == null) { // Caused by pressing CTRL+Z
